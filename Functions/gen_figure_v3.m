function gen_figure_v3(save_data,conn,table_name,default_parameters,system_names,configs,figure_data)
...
for primvar_sel = 1:length(primary_vals)
    ...
    for sel = 1:length(configs)
        ...
        switch conn.DataSource
            case "comm_database"
                result_label = params_inst.system_name;
            case "med_database"
                result_label = "BPDB";
        end

        % Edit parameters file if comm sim
        if conn.DataSource == "comm_database"
            switch result_label
                case "TODDM"
        ...
        switch conn.DataSource
            ...
            case "med_database"
                sqlquery = sprintf("SELECT * FROM %s WHERE param_hash = '%s' AND project_name = '%s'", ...
                    table_name, paramHash, result_label);
